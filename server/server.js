...
const generateKPIData = (range) => {
  ...
  for (let i = days - 1; i >= 0; i--) {
    ...
    kpiData.push({
      ...
      stock: Math.max(
        0,
        Math.round(totalStock * (1 + stockTrend + randomVariation))
      ),
      demand: Math.max(
        0,
        Math.round(totalDemand * (1 + demandTrend + randomVariation))
      ),
    });
  }
...
const typeDefs = `
  ...
  type Query {
    products(search: String, status: String, warehouse: String, range: String): [Product!]!
    warehouses(range: String): [Warehouse!]!
    kpis(range: String!, warehouse: String): [KPI!]!
  }
...
const resolvers = {
  Query: {
    products: (parent, args) => {
      ...
      // optional: simulate range filtering later
      return filteredProducts;
    },

    warehouses: (parent, args) => {
      // you can ignore args.range for now
      return warehouses;
    },

    kpis: (parent, args) => {
      let kpiData = generateKPIData(args.range);

      if (args.warehouse && args.warehouse !== 'All') {
        // recalc KPIs for only that warehouse
        ...
        const totalDemand = filteredProducts.reduce(
          (sum, p) => sum + p.demand,
          0
        );

        kpiData = kpiData.map((day) => ({
          ...day,
          stock: Math.round(totalStock * (1 + (Math.random() - 0.5) * 0.1)),
          demand: Math.round(totalDemand * (1 + (Math.random() - 0.5) * 0.1)),
        }));
      }
